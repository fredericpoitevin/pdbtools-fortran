c										
	subroutine compute(crd1,crd2,crdnew,nat,rmsd,r,ierr)
c	
	include 'param.h'
c	
	integer nat,ierr,i,j,k,iat,job,info
c	
	real*8 crd1(3*natmax),crd2(3*natmax),rmsd
	real*8 xcref(3),xcmov(3),crdnew(3*natmax)
	real*8 a(3,3),det,sign,d(3),work(3),u(3,3),v(3,3),work2(3)
	real*8 r(3,3),norm,c(3),tiny
c	
	real*8 deter
c	
c	FIND COM
...
100	continue
	do 101 iat = 1,nat
	  do 102 k = 1,3
	    xcref(k) = xcref(k) + crd1(3*(iat-1)+k)
	    xcmov(k) = xcmov(k) + crd2(3*(iat-1)+k)
102	  continue
101	continue
	do 103 k = 1,3
	  xcref(k) = xcref(k)/real(nat)
	  xcmov(k) = xcmov(k)/real(nat)
103	continue
c	
...
c	
	do 200 i = 1,3
	  do 201 j = 1,3
	    ...
	    do 202 iat = 1,nat
	      a(i,j) = a(i,j)
     1	  + (crd1(3*(iat-1)+i)-xcref(i))*(crd2(3*(iat-1)+j)-xcmov(j))
202	    continue
201	  continue
200	continue
c	
...
c	
c	PERFORM SVD ON COVAR MAT
c	
	call dsvdc(a,3,3,3,d,work,u,3,v,3,work2,11,info)
c	
	if(info.ne.0) then
	  ierr = 3
	  return
...
c	
c	COMPUTE BESTFIT ROTATIONAL MATRIX
c	
	if(d(2).gt.tiny) then
          if(d(3).le.tiny) then
            sign = 1.0d0
...
800       continue
          norm = sqrt(norm)
          if(norm.eq.0) then
            write(6,*) 'norm =',sngl(norm)
            write(6,*) 'Singular values :',(sngl(d(i)),i=1,3)
            write(6,*) 'vector u        :',(sngl(u(i,1)),i=1,3)
            write(6,*) 'vector v        :',(sngl(v(i,1)),i=1,3)
          endif
          do 900 i = 1,3
            work(i) = work(i)/norm
900       continue
...
c	
	det = deter(r)
	if(det.lt.0) then
	  write(6,*) 'Deter rotation negative and ierr = ',ierr
          write(6,*) 'Singular values :',(sngl(d(i)),i=1,3)
	endif
c	
c	COMPUTE RMSD
c	
        write(6,'("The rotation matrix is:")')
        do 998 j = 1,3
          write(6,'("ROTMAT> ",3f8.3)') r(j,1),r(j,2),r(j,3)
998     continue
	do 1100 i = 1,nat
	  do 1101 j = 1,3
	    c(j) = 0.d0
	    do 1102 k = 1,3
	      c(j) = c(j)
     1	           + r(j,k)*(crd2(3*(i-1)+k)-xcmov(k))
1102	    continue
1101	  continue
	  do 1103 j = 1,3
	    crdnew(3*(i-1)+j) = c(j) + xcref(j)
1103	  continue
1100	continue
	rmsd = 0.d0
	do 1200 i = 1,nat
	  do 1201 j = 1,3
	    rmsd = rmsd
     1	         + (crdnew(3*(i-1)+j)-crd1(3*(i-1)+j))**2
1201	  continue
1200	continue
	rmsd = sqrt(rmsd/real(nat))
c	
c	
	return
	end
...
c	
      ...
      double precision   dx(1), absxi,norm,scale,ssq,zero,one
