...
c	
	...
	real*8 uw(3*natmax,nmodmax),force(3*natmax)
        real*8 uwinv(3*natmax,nmodmax)
	real*8 norm(natmax),rmsdres(natmax)
c	
...
c									
	...
	write(6,*) " "
	call buildcrd(nmod,nat,crd,crdsave,crdave,rmsdres)
...
c									
	call builduwinv(crd,m,nmod,w,uwinv)
	call computef(uwinv,m,nmod,crdave,crdtar,force)
...
201         continue
200       continue
        call writepdb(crdforce,rmsdres,lleft,nat,2)
c	
c									
999	continue
	if(ierr.eq.1) then
...
c	
	...
	do 200 i = 1,m
     ...
     1	      + uw(i,j)*temp(j)
201	  continue
          forcenrm=sqrt(force(i)**2)
          if(forcenrm.gt.forcemax) then
            forcemax = forcenrm
          endif
200	continue
        do 300 i = 1,m
          force(i) = force(i)/forcemax
300     continue
c	
	return
	end
c									
	subroutine builduwinv(crd,m,nmod,w,uw)
c	
	include 'lrt.prm'
c	
	integer m,nmod,i,j,k
...
c	
c1	format(a30,3(f8.3),2(e12.5))
1       format(a30,3(f8.3),2(f6.2))
c	
        bmax = 0.d0
        do 200 i = 1,nat
          bftest = sqrt(bfac(i)**2)
          if(bftest.gt.bmax) then
            bmax = bftest
          endif
200     continue
	occ = 1.d0
...
c	  if(line(1:4).eq."ATOM".and.line(14:15).eq."CA") then
	    nat = nat + 1
	    if(nmod.eq.1) then
...
c									
c	
	subroutine buildcrd(nmod,nat,crd,crdsave,crdave,rmsdres)
c	
	include 'lrt.prm'
c	
...
c       
        do 400 i = 1,nat
          ...
          do 401 imod = 1,nmod
            rmsdtmp = 0.d0
            do 402 k = 1,3
              rmsdtmp = rmsdtmp + crd(3*(i-1)+k,imod)**2
402         continue
            rmsdres(i) = rmsdres(i) + sqrt(rmsdtmp)
401       continue
          rmsdres(i) = rmsdres(i)/real(nmod)
400     continue
c	
